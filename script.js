...
function loopObject(){
   for(let x in randomQoute){
       return "<b>Qoute</b> : " + randomQoute[x] + "<br>" +  "<b>Author</b> : " + x ;
       }
  
}

function displayQoutes():any {
   randomQoute = Qoutes[Math.floor(Math.random() * Qoutes.length)];
  return  document.getElementById("pOne").innerHTML = loopObject();
}


document.getElementById("myBtn").addEventListener("click", displayQoutes);
